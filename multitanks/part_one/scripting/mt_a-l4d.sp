...
int iMaxZombies, iFrustration[MAXPLAYERS+1], iMTHealthCoop[5], iMTHealthVersus[5], iMTHealthSurvival,
	iMTCountCoop[5], iMTCountVersus[5], iMTCountSurvival, iFinaleWave, iTankHP,
...
ConVar hMTOn, hMTHealthCoop[5], hMTHealthVersus[5], hMTHealthSurvival, hMTCountCoop[5],
	hMTCountVersus[5], hMTCountSurvival, hMTAnnounce, hMTSameSpawn[3],
	hMTSpawnDelay[2], hMTDisplay, hGameMode;
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	strcopy(error, err_max, "[MT] Plugin Supports L4D Only!");
...
public void OnPluginStart()
{
	...
	hMTHealthSurvival = CreateConVar("multitanks_health_survival", "15000", "Health Of Tanks (Survival)", FCVAR_NOTIFY|FCVAR_SPONLY);
	...
	for (int i = 0; i < 5; i++)
	{
		...
		switch (i)
		{
			case 0:
			{
				strcopy(sDescriptions[1], 128, "7500");
...
public void OnTankSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (bMTDisplay)
	{
		...
		for (int i = 1; i <= MaxClients; i++)
		{
			if (IsClientInGame(i) && GetClientTeam(i) == 3 && GetEntProp(i, Prop_Send, "m_zombieClass") == 5 && IsPlayerAlive(i) && !GetEntProp(i, Prop_Send, "m_isIncapacitated", 1))
			{
				char sText[128];
...
{
	return (client > 0 && client <= MaxClients && IsClientInGame(client) && GetClientTeam(client) == 3 && GetEntProp(client, Prop_Send, "m_zombieClass") == 5);
}
