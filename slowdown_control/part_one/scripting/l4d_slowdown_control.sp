...
public void OnPluginStart()
{
	...
	hCvarSdInwaterSurvivorRun = CreateConVar("l4d_slowdown_water_survivors_run", "140", "Maximum survivor Run speed in the water outside of Tank fights (0: don't modify speed; 220: default Survivor speed)", _, true, 0.0);
	hCvarSdInwaterSurvivorRunDuringTank = CreateConVar("l4d_slowdown_water_survivors_run_during_tank", "140", "Maximum survivor Run speed in the water during Tank fights (0: don't modify speed; 220: default Survivor speed)", _, true, 0.0);
	hCvarCrouchSpeedMod = CreateConVar("l4d_slowdown_crouch_speed_mod", "1.4", "Modifier of player crouch speed when inside a designated trigger, 75 is the defualt for everyone (1: default speed)", _, true, 0.0);
	...
	hCvarSdUziMod = CreateConVar("l4d_slowdown_uzi_percent", "0.0", "Unsilenced uzis cause this much slowdown * l4d_slowdown_gunfire at maximum damage.");
	...
	AutoExecConfig(true, "l4d_slowdown_control");
...
public void TankSpawn(Event event, const char[] name, bool dontBroadcast)
{
	if (!tankInPlay) 
	{
		...
		{
//			PrintToChatAll("\x04Water Slowdown\x01 has been reduced while Tank is in play.");
		}
	}
}
...
public Action Timer_CheckTank(Handle timer)
{
	...
	if (!tankclient || !IsPlayerAlive(tankclient)) 
	{
		...
		if (fSurvWaterRunSpeedDuringTank > 0.0 && g_bIsFirstTank) 
		{
//			PrintToChatAll("\x04Water Slowdown\x01 has been restored to normal.");
			g_bIsFirstTank = false;
		}
	}
	return Plugin_Stop;
		...
		case TEAM_SURVIVORS: 
		{
			...
			if (GetEntityFlags(client) & FL_INWATER) 
			{
				...
				{ // speed of survivors in water during Tank fights
					...
					{
						if(IsLimping(client) && fCvarSurvivorLimpRunSpeed < fSurvWaterRunSpeedDuringTank) 
						{ // not healthy
							return Plugin_Continue;
						}
				...
				{ // speed of survivors in water outside of Tank fights
					...
					{ // specific speed
						if(IsLimping(client) && fCvarSurvivorLimpRunSpeed < fSurvWaterRunSpeed) 
						{ // not healthy
							return Plugin_Continue;
						}
